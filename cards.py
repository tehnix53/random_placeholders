...
import numpy as np
from utility import random_backgrounds, random_white, create_bb, overlap, draw_border, random_file, small_list, \
    big_suits_dir, list_dir, extend_coordinate, rangs
...
def create_card(figure_numbers):
    '''
    ...
    for i in list_to_overlap:
        ...
        small_suit_file = random_file(small_list)
        suit = small_suit_file.split('/')[-2]
        big_pair_list = [i for i in list_dir(os.path.join(big_suits_dir, suit)) if i.endswith('.jpg')]
